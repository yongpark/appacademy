...
def substrings(string)
  arr = []
	split = string.chars
	split.each_with_index do |char1, idx1|
		split.each_with_index do |char2, idx2|
			arr << split[idx1..idx2].join
		end
	end
  arr
end
...
class Array
  def my_map(&prc)
    arr = []
    self.my_each {|num| arr << prc.call(num)}
    arr
  end
  ...
  def my_inject(&prc)
    val = self[0]
    self.drop(1).my_each {|num| val = prc.call(val, num)}
    val
  end
end
